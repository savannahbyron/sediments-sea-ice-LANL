...
#                IV_product_NH = np.sum(varIV.isel(nCells=NHindices), axis=(0,1))
                IV_product_NH = varIV.isel(nCells=NHindices).sum(dim=(None))
...
# Plot each variable
# Plot for NZ Product
axs[0].plot(timeNZ_product_NH[:, 0], timeNZ_product_NH[:, 1], label='NZ Product NH')
axs[0].plot(timeNZ_product_SH[:, 0], timeNZ_product_SH[:, 1], label='NZ Product SH')
axs[0].set_title('Vertical Aerosols Ice (NZ) Product', fontsize=16)
...
# Plot for RiverRunoff Freshwater Flux (RFW) Product
axs[1].plot(timeRFW_product_NH[:, 0], timeRFW_product_NH[:, 1], label='RFW Product NH')
axs[1].plot(timeRFW_product_SH[:, 0], timeRFW_product_SH[:, 1], label='RFW Product SH')
axs[1].set_title('RiverRunoff Freshwater Flux (RFW) Product', fontsize=16)
...
# Plot for Ice Volume (IV) Product
axs[2].plot(timeIV_product_NH[:, 0], timeIV_product_NH[:, 1], label='IV Product NH')
axs[2].plot(timeIV_product_SH[:, 0], timeIV_product_SH[:, 1], label='IV Product SH')
axs[2].set_title('Ice Volume (IV) Product', fontsize=16)
axs[2].legend(loc='upper right', fontsize=12)
axs[2].set_xticks(ticks)
axs[2].set_xticklabels(tick_labels, rotation=45, fontsize=12)
axs[2].set_xlim(start_year + start_month / 12, end_year + end_month / 12)
axs[2].set_xlabel('Time (Year-Month)', fontsize=14)

# Plot for NZ Aerosol Concentration (NZS) Product
axs[3].plot(timeNZS_product_NH[:, 0], timeNZS_product_NH[:, 1], label='NZS Product NH')
axs[3].plot(timeNZS_product_SH[:, 0], timeNZS_product_SH[:, 1], label='NZS Product SH')
axs[3].set_title('NZ Aerosol Concentration (NZS) Product', fontsize=16)
axs[3].legend(loc='upper right', fontsize=12)
axs[3].set_xticks(ticks)
axs[3].set_xticklabels(tick_labels, rotation=45, fontsize=12)
axs[3].set_xlim(start_year + start_month / 12, end_year + end_month / 12)
axs[3].set_xlabel('Time (Year-Month)', fontsize=14)
...
plt.tight_layout()
plt.savefig('OceanNotNormalNH_SH_Products.png')
plt.show()
