...
nCells = 465044
months = range(1, 5)  # Adjusted to include months 1-4
years = range(20, 21)  # Adjusted to include all
    ...
    for month in months:
        ...
        if os.path.exists(filepath_ice) & os.path.exists(filepath_ocean):
            ...
            if os.path.exists(output_filepath_ice) & os.path.exists(output_filepath_ocean):
                ...
                timeNZ_product_NH.append([year, month, np.sum(NZAER_product_NH)])
                ...
                timeNZ_product_SH.append([year, month, np.sum(NZAER_product_SH)])
                ...
                timeRFW_product_NH.append([year, month, np.sum(RiverrunoffFreshWaterFlux_product_NH)])
                ...
                timeRFW_product_SH.append([year, month, np.sum(RiverrunoffFreshWaterFlux_product_SH)])

                # ICEVOLUME
                IV_product_NH = np.sum(varIV.isel(nCells=NHindices), axis=(0, 1))
                timeIV_product_NH.append([year, month, np.sum(IV_product_NH)])
                IV_product_SH = np.sum(varIV.isel(nCells=SHindices), axis=(0, 1))
                timeIV_product_SH.append([year, month, np.sum(IV_product_SH)])
                ...
                # Append the results
                timeNZS_product_NH.append([year, month, np.sum(NZAER_ratio_NH)])
                timeNZS_product_SH.append([year, month, np.sum(NZAER_ratio_SH)])
...
timeNZ_product_NH_normalized = normalize(timeNZ_product_NH[:, 2])
timeNZ_product_SH_normalized = normalize(timeNZ_product_SH[:, 2])
timeRFW_product_NH_normalized = normalize(timeRFW_product_NH[:, 2])
timeRFW_product_SH_normalized = normalize(timeRFW_product_SH[:, 2])
timeIV_product_NH_normalized = normalize(timeIV_product_NH[:, 2])
timeIV_product_SH_normalized = normalize(timeIV_product_SH[:, 2])
timeNZS_product_NH_normalized = normalize(timeNZS_product_NH[:, 2])
timeNZS_product_SH_normalized = normalize(timeNZS_product_SH[:, 2])

# Define years and months based on your data
start_year = min(years)
end_year = max(years)

fig, axs = plt.subplots(2, 1, figsize=(24, 24), sharey=True)

# Create ticks for x-axis
ticks = [f'{year}-{month:02d}' for year in years for month in months]

# Plot NH data
axs[0].plot([f'{int(row[0])}-{int(row[1]):02d}' for row in timeNZ_product_NH], timeNZ_product_NH_normalized, label='NZ Product NH')
axs[0].plot([f'{int(row[0])}-{int(row[1]):02d}' for row in timeRFW_product_NH], timeRFW_product_NH_normalized, label='RFW Product NH')
axs[0].plot([f'{int(row[0])}-{int(row[1]):02d}' for row in timeIV_product_NH], timeIV_product_NH_normalized, label='IV Product NH')
axs[0].plot([f'{int(row[0])}-{int(row[1]):02d}' for row in timeNZS_product_NH], timeNZS_product_NH_normalized, label='NZS Product NH')
axs[0].set_title('Northern Hemisphere Products', fontsize=16)
axs[0].set_xticks(ticks)
axs[0].set_xticklabels(ticks, rotation=45, fontsize=12)
axs[0].set_xlim(0, len(ticks) - 1)
axs[0].set_xlabel('Time (Year-Month)', fontsize=14)

# Plot SH data
axs[1].plot([f'{int(row[0])}-{int(row[1]):02d}' for row in timeNZ_product_SH], timeNZ_product_SH_normalized, label='NZ Product SH')
axs[1].plot([f'{int(row[0])}-{int(row[1]):02d}' for row in timeRFW_product_SH], timeRFW_product_SH_normalized, label='RFW Product SH')
axs[1].plot([f'{int(row[0])}-{int(row[1]):02d}' for row in timeIV_product_SH], timeIV_product_SH_normalized, label='IV Product SH')
axs[1].plot([f'{int(row[0])}-{int(row[1]):02d}' for row in timeNZS_product_SH], timeNZS_product_SH_normalized, label='NZS Product SH')
axs[1].set_title('Southern Hemisphere Products', fontsize=16)
axs[1].set_xticks(ticks)
axs[1].set_xticklabels(ticks, rotation=45, fontsize=12)
axs[1].set_xlim(0, len(ticks) - 1)
...
plt.savefig('month1_4_NH_SH_Products_Normalized.png')
plt.show()
