...
timeAC_product_NH_normalized = normalize(timeAC_product_NH[:, 1])
timeAC_product_SH_normalized = normalize(timeAC_product_SH[:, 1])
timeRFWnoAC_product_NH_normalized = normalize(timeRFWnoAC_product_NH[:, 1])
timeRFWnoAC_product_SH_normalized = normalize(timeRFWnoAC_product_SH[:, 1])
...
tick_labels = [f'{month_names[month-1]}' for year in range(start_year, end_year + 1) for month in range(start_month, end_month + 1)]

variables = {
    'Total Sediment': (timeNZ_product_NH, timeNZ_product_NH_normalized, 'magenta'),
    'River Freshwater Flux': (timeRFW_product_NH, timeRFW_product_NH_normalized, 'green'),
    'Ice Volume': (timeIV_product_NH, timeIV_product_NH_normalized, 'red'),
    'Area Cell': (timeAC_product_NH, timeAC_product_NH_normalized, 'brown'),
    'River Freshwater Flux No AC': (timeRFWnoAC_product_NH, timeRFWnoAC_product_NH_normalized, 'pink'),
    'Sediment Concentration': (timeNZS_product_NH, timeNZS_product_NH_normalized, 'purple'),
}

for var_name, (time_data, normalized_data, color) in variables.items():
    plt.figure(figsize=(18, 10))
    plt.plot(time_data[:, 0], normalized_data, label=var_name, linewidth=2, color=color)
    plt.title(f'{var_name} Over Time', fontsize=24)
    plt.xticks(ticks, tick_labels, rotation=45, fontsize=16)
    plt.xlim(start_year + start_month / 12, end_year + end_month / 12)
    plt.xlabel('Time (Month-Year)', fontsize=20)
#    plt.ylabel('Normalized Values', fontsize=20)
#    plt.legend(fontsize=16)
#    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'{var_name}_Normalized.png')
    plt.close()
