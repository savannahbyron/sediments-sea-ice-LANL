...
# Save sum_over_months to pickle file after all data processing is complete
pickle_filename = 'sum_over_months.pkl'
with open(pickle_filename, 'wb') as f:
    pickle.dump(sum_over_months, f)
    print(f'Saved {pickle_filename} to pickle file')
...
total_months = (41 - 10) * 12
xaxis = np.arange(0, total_months) / 12

time_sum_array = np.zeros((len(years) * 12))

index = 0
for year in years:
    for month in months:
        time_sum_array[index] = np.sum(sum_over_months[month - 1])
        index += 1
