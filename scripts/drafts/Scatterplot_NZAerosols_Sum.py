...
# Add var name to the center of the plot
    fig.text(0.5, 0.5, f'Year_30_{varName}', ha='center', va='top', fontsize=16)
...
# Inside the plotting loop
    plt.savefig(os.path.join(output_folder, f'Lat_Long_seaice_Output_Year30_{varName}.png'))
...
# Save the figure with a dynamic filename including year
plt.savefig(f'Lat_Long_seaice_Output_Year30_{varName}.png')
plt.close(fig)  # Close the figure to free up memory
