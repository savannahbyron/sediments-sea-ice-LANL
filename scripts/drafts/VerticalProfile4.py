            ...
            if os.path.exists(output_filepath_ice) & os.path.exists(output_filepath_ocean):
                ...
                def calculate_totals(layers, indices):
                    return np.sum(varNZ.isel(nCells=indices, nzAerosolsIceLayers=layers) *
                                  ...
                                  areaCell.isel(nCells=indices), axis=(1, 2))
                ...
                # Freshwater
                timeRFW_product_NH.append([year + month / 12, np.sum(varRFW.isel(nCells=NHindices) * varAC.isel(nCells=NHindices) * areaCell.isel(nCells=NHindices), axis=(0, 1))])
                timeRFW_product_SH.append([year + month / 12, np.sum(varRFW.isel(nCells=SHindices) * varAC.isel(nCells=SHindices) * areaCell.isel(nCells=SHindices), axis=(0, 1))])

                # NZAEROSOL CONCENTRATION
                NZS_product_NH = np.sum(varNZ.isel(nCells=NHindices), axis=(0, 1))
                total_volume_NH = np.sum(varIV.isel(nCells=NHindices), axis=(0, 1))
                NZAER_ratio_NH = NZS_product_NH / total_volume_NH

                NZS_product_SH = np.sum(varNZ.isel(nCells=SHindices), axis=(0, 1))
                total_volume_SH = np.sum(varIV.isel(nCells=SHindices), axis=(0, 1))
...
def plot_data(time_data, rfw_data, title, filename):
    ...
    # NZ Aerosols plot
    axs[0].plot(time_data[:, 0], time_data[:, 1], label='Layers 0-7', color='blue')
    axs[0].fill_between(time_data[:, 0], 0, time_data[:, 1], color='blue', alpha=0.3)
    axs[0].plot(time_data[:, 0], time_data[:, 2], label='Layers 8-16', color='green')
    axs[0].fill_between(time_data[:, 0], 0, time_data[:, 2], color='green', alpha=0.3)
    axs[0].plot(time_data[:, 0], time_data[:, 3], label='Layers 17-24', color='red')
    axs[0].fill_between(time_data[:, 0], 0, time_data[:, 3], color='red', alpha=0.3)
    ...
    axs[2].plot(time_data[:, 0], time_data[:, 1], label='Layers 0-7', color='blue')
    axs[2].fill_between(time_data[:, 0], 0, time_data[:, 1], color='blue', alpha=0.3)
    axs[2].plot(time_data[:, 0], time_data[:, 2], label='Layers 8-16', color='green')
    axs[2].fill_between(time_data[:, 0], 0, time_data[:, 2], color='green', alpha=0.3)
    axs[2].plot(time_data[:, 0], time_data[:, 3], label='Layers 17-24', color='red')
    axs[2].fill_between(time_data[:, 0], 0, time_data[:, 3], color='red', alpha=0.3)
...
# Plot data for NH
plot_data(timeNZ_product_NH, timeRFW_product_NH, 'Northern Hemisphere', 'VertProfile1NH_Products.png')

# Plot data for SH
plot_data(timeNZ_product_SH, timeRFW_product_SH, 'Southern Hemisphere', 'VertProfile2SH_Products.png')
